...
# tau = 1
# model_org = LIM(tau)
# model_org.fit(raw_data.detach().cpu().numpy())
# #131999 = 11000 years
# lim_integration, times_ = model_org.noise_integration(raw_data[:, -1], timesteps=timesteps, num_comp=30)
...
tau = 1
model1 = LIM(tau)
model1.fit(data[:, :14000].detach().cpu().numpy())

model2 = LIM(tau)
model2.fit(data[:, :13000].detach().cpu().numpy())

model3 = LIM(tau)
model3.fit(data[:, :12000].detach().cpu().numpy())

model4 = LIM(tau)
model4.fit(data[:, :11000].detach().cpu().numpy())

model5 = LIM(tau)
model5.fit(data[:, :10000].detach().cpu().numpy())

model6 = LIM(tau)
model6.fit(data[:, :9000].detach().cpu().numpy())

model7 = LIM(tau)
model7.fit(data[:, :8000].detach().cpu().numpy())

model8 = LIM(tau)
model8.fit(data[:, :7000].detach().cpu().numpy())

#131999 = 11000 years
lim_integration1, times_ = model1.noise_integration(data[:, 14000], timesteps=timesteps, num_comp=30)
lim_integration1 = torch.from_numpy(lim_integration1.T)

lim_integration2, times_ = model2.noise_integration(data[:, 13000], timesteps=timesteps, num_comp=30)
lim_integration2 = torch.from_numpy(lim_integration2.T)

lim_integration3, times_ = model3.noise_integration(data[:, 12000], timesteps=timesteps, num_comp=30)
lim_integration3 = torch.from_numpy(lim_integration3.T)

lim_integration4, times_ = model4.noise_integration(data[:, 11000], timesteps=timesteps, num_comp=30)
lim_integration4 = torch.from_numpy(lim_integration4.T)

lim_integration5, times_ = model5.noise_integration(data[:, 10000], timesteps=timesteps, num_comp=30)
lim_integration5 = torch.from_numpy(lim_integration5.T)

lim_integration6, times_ = model6.noise_integration(data[:, 9000], timesteps=timesteps, num_comp=30)
lim_integration6 = torch.from_numpy(lim_integration6.T)

lim_integration7, times_ = model7.noise_integration(data[:, 8000], timesteps=timesteps, num_comp=30)
lim_integration7 = torch.from_numpy(lim_integration7.T)

lim_integration8, times_ = model8.noise_integration(data[:, 7000], timesteps=timesteps, num_comp=30)
...
torch.save(lim_integration, "lim_integration_multipleLim_XLim_80k.pt")
