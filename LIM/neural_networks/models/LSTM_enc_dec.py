...
class LSTM_Encoder(nn.Module):
    ...
    def forward(self, x_input):
        ...
        lstm_out, self.hidden = self.lstm(x_input.view(x_input.shape[2], x_input.shape[0] , self.input_size ))
...
class LSTM_Decoder(nn.Module):
    ...
    def forward(self, decoder_input, decoder_hidden, outputs=None, target_batch=None, training_prediction=None, target_len=None, teacher_forcing_ratio=None, prediction_type=None):
        ...
        #lstm_out, self.hidden = self.lstm(decoder_input.unsqueeze(0), decoder_hidden)
        #output = self.linear(lstm_out.squeeze(0))
        #print(decoder_input.shape, decoder_hidden[0].shape)
        # hidden = torch.flatten(encoder_hidden_states[0], end_dim=1)
        # print(hidden.shape)
        # hidden = hidden.unsqueeze(1)
        # print(hidden.shape)
        # hidden = hidden.reshape(1, x_input.shape[0], x_input.shape[1])
        # print(hidden.shape)
        # print(x_input.unsqueeze(0).shape)
        #
        # lstm_out, self.hidden = self.lstm(hidden, encoder_hidden_states)
        # output = self.linear(lstm_out.squeeze(0))
...
class LSTM_Decoder_WO(nn.Module):
    """
    Decodes hidden state output by encoder
    """
    ...
    def __init__(self, input_size, hidden_size, num_layers=3):
        ...
        super(LSTM_Decoder_WO, self).__init__()
        self.input_size = input_size
        self.hidden_size = hidden_size
        self.num_layers = num_layers
        self.lstm = nn.LSTM(input_size=input_size, hidden_size=hidden_size,
...
class LSTM_Sequence_Prediction(nn.Module):
    ...
    def predict(self, input_tensor, target_len, prediction_type='test'):
        ...
        for t in range(target_len):
            decoder_output, decoder_hidden = self.decoder(decoder_input, decoder_hidden, prediction_type=prediction_type)
            if prediction_type == 'forecast':
